	...
	, m_pLevel(nullptr)
{
	...
	m_LevelNames.push_back("LevelPistonTest.txt");
}
...
bool GameplayState::Load()
{
	...
	return m_pLevel->Load(m_LevelNames.at(3), m_player.GetXPositionPointer(), m_player.GetYPositionPointer());
...
ECollisionEvent GameplayState::HandleCollision(int newPlayerX, int newPlayerY)
{
	PlacableActor* collidedActor = m_pLevel->UpdateActors(newPlayerX, newPlayerY);
	ECollisionEvent e = ECollisionEvent::None;
	...
	if (collidedActor != nullptr && collidedActor->IsActive())
	{
		...
		{
			Piston* collidedPiston = dynamic_cast<Piston*>(collidedActor);
			assert(collidedPiston);
			int Travel_X, Travel_Y;
			Travel_X = m_player.GetXPosition() - collidedPiston->GetXPosition();
			Travel_Y = m_player.GetYPosition() - collidedPiston->GetYPosition();
			for (int i = 1; i < 20; i++)
			{
				ECollisionEvent PathCollision = HandleCollision(newPlayerX+(i* Travel_X), newPlayerY+(i*Travel_Y));
				if (PathCollision > ECollisionEvent::Overlap)
				{
					e = PathCollision;
					break;
				}
